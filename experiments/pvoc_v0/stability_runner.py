...
from .messages import directed_pairs
from .metrics import immediate_utility
...
def condition_order(trial_index: int) -> tuple[StabilityCondition, StabilityCondition]:
    if trial_index % 2:
...
def _action_counts(records: list[StabilityTrialRecord]) -> dict[str, int]:
...
def _modal_action(counts: dict[str, int]) -> str:
    if not counts:
        raise StabilityValidationError("Cannot compute a modal action from no trials.")
...
def _condition_summary(records: list[StabilityTrialRecord]) -> dict[str, object]:
    if len(records) != N_TRIALS:
        raise StabilityValidationError(f"Expected {N_TRIALS} trials per condition, got {len(records)}.")
    counts = _action_counts(records)
    modal = _modal_action(counts)
    return {
        "action_counts": counts,
        "modal_action": modal,
        "modal_share": counts[modal] / N_TRIALS,
...
def summarize_pair_records(records: list[StabilityTrialRecord]) -> dict[str, object]:
    ...
    first = records[0]
    delta = mean(record.utility for record in with_message) - mean(
        record.utility for record in without
    )
    return {
        "case_id": first.case_id,
        "sender": first.sender,
        "recipient": first.recipient,
        "without_message": _condition_summary(without),
        "with_message": _condition_summary(with_message),
